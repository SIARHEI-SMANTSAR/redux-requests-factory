...
const createSelectors = <Resp, Err, Params, State, TransformedResp, Key extends string>(
  ...
  if (isWithSerialize<Resp, Err, Params, State, TransformedResp>(factoryConfig)) {
    const { serializeRequestParameters } = factoryConfig;
    return {
      responseSelector: createSelector([getCommonSate], (commonSate) => (params: Params) => {
        const response = getByPath<Resp, RequestsState | undefined>(
          serializeRequestParameters(params),
          "response",
        )(commonSate);

        if (isWithTransformResponse<Resp, Err, Params, State, TransformedResp>(factoryConfig)) {
          return factoryConfig.transformResponse(response);
        }

        return response;
      }),
      errorSelector: createSelector([getCommonSate], (commonSate) => (params: Params) => {
      ...
      requestStatusSelector: createSelector([getCommonSate], (commonSate) => (params: Params) => {
      ...
      isLoadingSelector: createSelector([getCommonSate], (commonSate) => (params: Params) => {
        const status = getByPath<RequestsStatuses, RequestsState | undefined>(
          serializeRequestParameters(params),
          "status",
        )(commonSate);
      ...
      isLoadedSelector: createSelector([getCommonSate], (commonSate) => (params: Params) => {
        const status = getByPath<RequestsStatuses, RequestsState | undefined>(
          serializeRequestParameters(params),
          "status",
        )(commonSate);
    ...
    } as RequestsFactoryItemSelectors<Resp, Err, Params, State, TransformedResp>;
  } else {
    ...
    return {
      responseSelector: isWithTransformResponse<Resp, Err, Params, State, TransformedResp>(
        factoryConfig,
      )
      ...
      requestStatusSelector: createSelector([getCommonSate], (commonSate) => {
        const status = getByPath<RequestsStatuses, RequestsState | undefined>("status")(commonSate);
